...
def extract_outline(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- NEW: Merge adjacent word spans into lines ---
        
        # Group spans by (page, y position rounded to nearest pixel)
        line_groups = defaultdict(list)
        for block in all_blocks:
            page = block["page"]
            # Use the vertical center of the bbox as the y position
            y_center = int((block["bbox"][1] + block["bbox"][3]) / 2)
            line_groups[(page, y_center)].append(block)
        
        merged_blocks = []
        for (page, y), blocks in line_groups.items():
            # Sort by x0 (left position)
            blocks = sorted(blocks, key=lambda b: b["bbox"][0])
            # Merge text and compute average font size and bounding box
            merged_text = " ".join(b["text"] for b in blocks)
            avg_size = np.mean([b["size"] for b in blocks])
        ...
        # Replace all_blocks with merged_blocks
        all_blocks = merged_blocks
        
        # Get all font sizes for relative sizing
        font_sizes = [block["size"] for block in all_blocks]
        
        # --- Filter out single-word or very short lines (unless very large font) ---
        filtered_blocks = []
        for block in all_blocks:
            words = block["text"].split()
            if len(words) >= 2 or block["size"] >= (max(font_sizes) * 0.85):
